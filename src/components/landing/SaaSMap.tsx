...
export default function SaaSMap() {
  ...
  const bottomText = isRu
    ? "Ниже — цепочка шагов от 1 до 7."
    : "Below is the chain of steps from 1 to 7.";
  ...
  return (
    <Section id="saas-map" className="relative overflow-hidden py-16 sm:py-24">
      {/* ✅ ВАЖНО: НЕТ “BG” (чёрного фона) — иначе он перекрывает видео из VideoBgGroup */}

      {/* ✅ Оверлеи читаемости поверх общего видео */}
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0">
        {/* общий затемняющий градиент */}
        <div className="absolute inset-0 bg-[linear-gradient(to_bottom,rgba(0,0,0,0.22),rgba(0,0,0,0.92))]" />

        {/* акценты (оранжевые дымки, но прозрачные) */}
        <div
          className="absolute inset-0"
          style={
            {
              background:
                "radial-gradient(900px_520px_at_12%_18%,rgba(255,154,61,0.10),transparent_68%)," +
                "radial-gradient(820px_520px_at_86%_30%,rgba(255,106,26,0.08),transparent_66%)",
        ...
        <div
          ...
          style={
            {
              backgroundImage:
                "radial-gradient(circle at 1px 1px, rgba(255,255,255,0.22) 1px, rgba(0,0,0,0) 1.8px)",
        ...
        {/* ✅ низ уводим в #050505 для стыка с FAQ */}
        <div
        ...
        <Container>
          <div ref={rootRef} className="relative">
            ...
            >
              ...
              <div className="mt-4 font-display text-[28px] leading-[1.08] sm:text-[40px] font-extrabold text-white">
                {title}
              </div>
            </div>
            ...
            <div className="mt-10 sm:mt-12">
                  ...
                  <ol className="relative grid grid-cols-7 gap-2 sm:gap-3">
                    {STEPS.map((s) => (
                      <li key={s} className="flex flex-col items-center">
                        <div
                          className="flex h-8 w-8 items-center justify-center rounded-full text-[12px] font-semibold text-[#F9EEE5]"
                          style={
                            {
                              background:
              ...
              <div className="mx-auto mt-5 max-w-3xl text-center text-[12px] text-white/55">
                {bottomText}
              </div>
            </div>
          </div>
