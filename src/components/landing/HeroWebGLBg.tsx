...
function Quad({ onContextLost }: { onContextLost: () => void }) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const y = clamp01(1.0 - (e.clientY - r.top) / Math.max(1, r.height));
  ...
  useFrame((_, delta) => {
    ...
    const pm = matRef.current.uniforms.uPrevMouse.value as THREE.Vector2;

    pm.copy(prevMouse.current);

    const lerpK = 1.0 - Math.pow(0.001, dt);
