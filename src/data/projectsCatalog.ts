  ...
  return [
      // 0) TIVONIX PANEL — партнёрская панель
      {
        ...
        detailsRu:
          ...
          "Зачем это\n" +
          "Партнёрство редко разваливается из‑за условий. Оно сыпется, когда **никто не видит картину**: где заявка, на каком этапе сделка, когда выплата. Пока правда живёт в Телеграме и Excel — каждый день начинается с «напомни» и скринов в полночь.\n\n" +
          ...
          "Как работает\n" +
          "Партнёр регистрируется, выбирает модель — **реферальная** или **под своим брендом** — и после модерации получает доступ в кабинет.\n" +
          ...
          "В **партнёрах** — сеть целиком: активность, клиенты, сделки, продажи, комиссия и баланс. Отдельно — заявки на вход (реферальная модель / работа под своим брендом) и модерация. **Выплаты** и комиссии живут в панели, без сторонних таблиц. Интерфейс собран под ежедневную работу, а не под презентацию.\n\n" +
          "Что сделали\n" +
          "Разработка TIVONIX — **1 неделя**. Спроектировали структуру под реальный партнёрский процесс, собрали регистрацию, вход и сделки, довели интерфейс (сетка, статусы, **пустые состояния**) и вывели в работу на Railway.\n\n" +
          "Итог\n" +
        ...
        outcomes: [
          ...
          isRu
            ? "**Реферальная** модель и работа **под своим брендом**"
            : "**Referral** and **White-label** models",
          isRu
            ? "Продукт **уже онлайн** на Railway"
      ...
      // 1b) LOGOVO — сеть шиномонтажа · https://www.logovo24.by/
      {
        ...
        detailsRu:
          ...
          "Запуск под ключ\n" +
          "Помогли с **доменом logovo24.by**, **сами** подняли хостинг (**hoster.by** / cPanel), выгрузили статику `out/`, настроили боевую среду. Полный цикл: идея → Figma → код → запуск.\n\n" +
          "Итог\n" +
      ...
      // 2) UPC — SaaS MVP (client: ИП Безбородых И.В.) · https://upc.watch/
      {
        ...
        detailsRu:
          ...
          "• Серверная часть на Supabase/Postgres под учёт, интеграции и рост функциональности\n\n" +
          "Результат\n" +
      ...
      // 3) PAYCLIP — 2 weeks (client: ИП Безбородых И.В.)
      {
        ...
        detailsRu:
          ...
          "• Спроектировали структуру под заявки: предложение → доверие → сценарии → призыв к действию\n" +
          "• Собрали чистый интерфейс: сетка, отступы, контраст, типографика\n" +
          "• Протянули ключевые пользовательские сценарии (первые шаги, ввод в работу)\n" +
      ...
      // 4) HEADMIND — корпоративный сайт на WordPress
      {
        ...
        detailsRu:
          ...
          "Дизайн-направление, реализация на WordPress, хостинг, домен, запуск и передача проекта.\n\n" +
      ...
      // 7) SLOTTY — маркетплейс онлайн-записи к мастерам
      {
        ...
        title: "Slotty",
        subtitleRu:
          "Маркетплейс онлайн-записи к мастерам: каталог с фильтрами и картой, мини-приложение в Телеграме, кабинет мастера (тарифы Free/Pro), админка платформы, bePaid — на Railway, домен slotty.of.by.",
        subtitleEn:
        ...
        detailsRu:
          ...
          "Задача\n" +
          "Запись часто идёт через личные сообщения и мессенджеры: клиент и мастер теряют время, слоты не видны. Нужны каталог, фильтры, карта, кабинет мастера, роли, админка платформы и оплаты в одной системе.\n\n" +
          "Что сделала TIVONIX\n" +
          "Дизайн и разработка под ключ: клиентский каталог с фильтрами и картой, мини-приложение в Телеграме, кабинет мастера (тарифы Free/Pro), админка платформы, интеграция **bePaid**, домен **slotty.of.by**, публикация на **Railway** (сайт и API).\n\n" +
          "Зона ответственности TIVONIX\n" +
          "Продуктовая архитектура, UI/UX, фронтенд и бэкенд, база данных, интеграции, инфраструктура и выкладка на домен заказчика.\n\n" +
          "Подтверждённый результат\n" +
          "Рабочая платформа на slotty.of.by: каталог, запись по слотам, кабинеты мастера и администратора, мини-приложение в Телеграме.\n\n" +
          "Технологии\n" +
        ...
        outcomes: [
          ...
          isRu
            ? "Кабинет мастера и админка платформы"
      ...
      // 9) SPLITON — финтех-платформа для музыкальных активов
      {
        ...
        title: "Spliton",
        subtitleRu:
          "Финтех-платформа для долей в музыке: каталог, первичный и вторичный рынок, кошелёк USDT, учёт операций, комплаенс и портал оператора — продукт с сопровождением TIVONIX.",
        subtitleEn:
        ...
        detailsRu:
          ...
          "Что сделала TIVONIX\n" +
          "Кабинет инвестора, портал оператора, реестр операций, казначейство, KYC/AML, вторичный рынок, публичный раздел о прозрачности, автотесты на критичные денежные сценарии. Дизайн, фронтенд (Next.js), бэкенд (NestJS), база (PostgreSQL/Prisma), публикация и операционные процедуры.\n\n" +
          "Зона ответственности TIVONIX\n" +
          "Продуктовая архитектура, UI/UX, разработка, комплаенс-контуры в продукте, тестирование и сопровождение после запуска.\n\n" +
          "Подтверждённый результат\n" +
          "Платформа на spliton.io: каталог релизов, покупка долей, кошелёк USDT (TRC20), вторичный рынок, портал оператора с финансовыми и операционными разделами.\n\n" +
          "Технологии\n" +
        ...
        roleRu: "Продуктовая архитектура, UX/UI, фронтенд, бэкенд, комплаенс-сценарии, тестирование и поддержка",
        roleEn: "Product architecture, UX/UI, frontend, backend, compliance flows, testing and support",
        seoTitleRu: "Spliton — финтех-платформа с первичным и вторичным рынком | TIVONIX",
        seoTitleEn: "Spliton — FinTech platform with primary and secondary market | TIVONIX",
        seoDescriptionRu:
          "Spliton — финтех-продукт TIVONIX: каталог музыкальных релизов, покупка долей, кошелёк USDT, реестр операций, KYC/AML, вторичный рынок и портал оператора.",
        seoDescriptionEn:
        ...
        outcomes: [
          isRu
            ? "Кабинет инвестора + вторичный рынок + портал оператора"
            : "Investor portal + secondary market + operator portal",
          isRu
            ? "Портал оператора: выплаты, казначейство, комплаенс"
        ...
        testimonial: {
          ...
          text: isRu
            ? "У Spliton тяжёлая начинка: доли, кошелёк, выплаты, портал оператора. Собрали целиком, выкатили и остались на сопровождении."
